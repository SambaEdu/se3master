...
for ($i = 0; $i < $count; $i++) {
	...
	if ($testniveau < "3") {
		...
		if ($file<>'.' and $file<>'..' and $file<>'registre.vbs' and $file<>'skeluser') {
			if ((this_parc_delegate($login,$file,'manage')) or (is_admin("computers_is_admin",$login)=="Y")) { echo "<a href=\"affiche_restrictions.php?salles=$file\">$file </a><br>"; $test_affiche++;}
               	}  
	}
}
