...
REPORT_FILE=$LOG_DIR/log_maj104
...
if [ ! -z "$MAIL_ADMIN" ]; then
	...
	echo "$REPORT"  | mail -s "[SE3] Résultat de la Mise a jour 104" $MAIL_ADMIN
fi
}
...
echo "Mise a jour 104:
- Correction bug udateClass.pl
- Correction var post sur remplacant et creation groupes - s groupes" >> $HISTORIQUE_MAJ
MAIL_REPORT
