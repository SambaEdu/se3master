...
REPORT_FILE=$LOG_DIR/log_maj108
...
if [ ! -z "$MAIL_ADMIN" ]; then
	...
	echo "$REPORT"  | mail -s "[SE3] Résultat de la Mise a jour 108" $MAIL_ADMIN
fi
}
...
echo "Mise a jour 108:
- Correction test.php sur le test de la maj disponible
- Correction sur restorebackup.sh pour NAS
- Ajout test appartenance sur Equipe / Classes pour droit sovajonisadmin afin d'éviter les groupes cours" >> $HISTORIQUE_MAJ
MAIL_REPORT
