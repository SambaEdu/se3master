...
Ce script est distribu� selon les termes de la licence GPL
...
function der_inventaire($nom_machine) { // retourne la date du dernier inventaire a partir de hardware
	...
	if ($result) {
		...
		return $retour;
	} else { // Pas d'inventaire � ce nom
		return 0;
	}	
}

function avoir_systemid($nom_machine) { // retourne l'ID de $nom_machine ou 0 � partir de la table hardware
        include "dbconfig.inc.php";
	$dbnameinvent="ocsweb";
