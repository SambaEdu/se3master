...
switch($act) {
	...
	case "maj":
	$content_dir = '/tmp/';
	$fichier_xml = $content_dir."mod.xml";
	if (!$ftpmaj) {$ftpmaj="ftp://193.49.66.139/pub/sambaedu";}
	//echo "Connexion a $urlxmlgroups ";
	//$fd = fopen ($urlxmlgroups, "r");
	if (file_exists($fichier_mod_xml)) unlink($fichier_mod_xml);

	if (file_exists("/etc/se3/www-tools/mod.xml.gz")) {
		system("cp /etc/se3/www-tools/mod.xml.gz /tmp/; mv /etc/se3/www-tools/mod.xml.gz /etc/se3/www-tools/mod.xml.gz.done", $retval_mod);
		echo "<strong>".gettext("Utilisation du fichier local")." /etc/se3/www-tools/mod.xml.gz ".gettext("pour la premi&#232;re importation")." </strong><br>";
		system ("cd /tmp;  gunzip mod.xml.gz");
	} else {
		echo gettext("Connexion &#224")." $ftpmaj <br>";
		system("cd /tmp; wget -q $ftpmaj/mod.xml.gz", $retval_mod);
		if ("$retval_mod") {
			echo gettext("Une erreur est survenue lors du t&#233l&#233chargement de l'archive mod.xml.gz<BR>V&#233rifiez votre connexion internet");
			// echo "Valeur de retour : $retval_mzi";	
			echo "<BR><BR><BR><div class=\"alert_msg\">".gettext("La mise &#224 jour des groupes de cl&#233s a &#233chou&#233e !")."</div>";
		} else {
			echo gettext("T&#233l&#233chargement de l'archive mod.xml sur le serveur SE3 effectu&#233...")."<br>";
			system ("cd /tmp;  gunzip mod.xml.gz");
		}
	}
