...
if (is_admin("computers_is_admin",$login)=="Y") {
	...
	if (! isset($tstlogin)) {
        	...
        	echo "<TR><TD>".gettext("Nom netbios du client")."</TD><TD><INPUT TYPE=text NAME=tstnetbios></TD></TR>\n";
...
if ($test) {
	...
	if (($testniveau==2) and !($sscat)) { 
        ...
        if (mysql_num_rows($resultat)) {
        	echo "<table border=\"1\"><tr><td><img src=\"/elements/images/help.png\" alt=\"".gettext("Aide")."\" title=\"".gettext("Aide")."\" width=\"16\" height=\"18\" border=\"0\" /></td><td>".gettext("Intitul&#233")."</td><td>".gettext("Valeur")."</td><td>".gettext("Template")."</td></tr>";
	
		while ($row=mysql_fetch_array($resultat)) {
               		$color="";
               		if ($row[2]=="restrict") { $color="#FF0000"; }
               		$testvaleur=$row[3];
               		$testemplate="base";
               		for ($i=0;$i<count($templates);$i++) {
                		...
                		if (mysql_num_rows($resultat2)) {
               				...
               				if ($row[2]=="restrict") { $color="#00FF00"; }
               				$testvaleur=$row2[0];
                	...
                	echo "<tr><td><a href=\"aide_cle.php?cle=$row[0]\" target=\"_blank\" ><img src=\"/elements/images/help.png\" alt=\"".gettext("aide")."\" title=\"$row[1]\" width=\"16\" height=\"18\" border=\"0\" /></a>";
