...
switch($act) {
	default:
	echo "<a href=\"cle-maj.php?action=maj\">".gettext("Effectuer la mise &#224 jour des restrictions ?")."</a>";
	break;
	...
	if (!$ftpmaj) {$ftpmaj="ftp://193.49.66.139/pub/sambaedu";}
	
	$fichier_xml = $content_dir . "rules.xml";
	if (file_exists("/tmp/rules.gz")) unlink("/tmp/rules.xml.gz");

	if (file_exists("/etc/se3/www-tools/rules.xml.gz")) {
		system("cp /etc/se3/www-tools/rules.xml.gz /tmp/rules.xml.gz; mv /etc/se3/www-tools/rules.xml.gz /etc/se3/www-tools/rules.xml.gz.done", $retval_rules);
		echo "<strong>".gettext("Utilisation du fichier local")." /etc/se3/www-tools/rules.xml.gz ".gettext("pour la premi&#232re importation")." </strong><br>";
		system ("cd /tmp;  gunzip rules.xml.gz");
	} else	{
		echo gettext("Connexion &#224;")." $ftpmaj <br>";
		system("cd /tmp; wget -q $ftpmaj/rules.xml.gz", $retval_rules);
		
		if ("$retval_rules") {
			echo gettext("Une erreur est survenue lors du t&#233;l&#233;chargement de l'archive")." rules.xml.gz<BR>".gettext("V&#233rifiez votre connexion internet");
			// echo "Valeur de retour : $retval_mzi";	
			echo "<BR><BR><BR><div class=\"alert_msg\">".gettext("La mise &#224; jour des cl&#233s a &#233;chou&#233;e !")."</div>";
		} else {
			echo gettext("T&#233;l&#233;chargement de l'archive rules.xml sur le serveur SE3 effectu&#233;...")."<br>";
			system ("cd /tmp;  gunzip rules.xml.gz");
...
// 
	...
	if (!($id_fichier = fopen($fichier_xml, "r"))) {
  	...
  	unlink($fichier_xml);
    	...
    	$contenu=preg_replace($patterns, $replacements, $ligne);
    	$patterns[0] = "/252/";
    	$patterns[1] = "/12/";
    	$patterns[2] = "/224/";
    	$patterns[3] = "/8/";
    	...
    	$replacements[0] = "TOUS";
    	$replacements[1] = "TypeXP";
    	$replacements[2] = "Type9x";
    	$replacements[3] = "2K";
    	$replacements[4] = "XP";
    	$liste=preg_replace($patterns, $replacements, $contenu);
