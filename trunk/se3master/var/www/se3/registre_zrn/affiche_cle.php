...
$mod=$_GET['mod'];
if (!$lien_retour) { $lien_retour=$_POST['lien_retour'];}
...
switch ($modif)
{
	...
	if (($clemodif) and (!$suppr)) {
    		...
    		$row = mysql_fetch_array($resultat);
     		...
     		echo"<H3>";
        	echo "<a href=\"#\" onClick=\"window.open('aide_cle.php?cle=$row[1]','aide','scrollbars=yes,width=600,height=620')\"><img src=\"/elements/images/help.png\" alt=\"aide\" title=\"$row[7]\" width=\"15\" height=\"15\" border=\"0\">$row[0]</a></H3>";
	...
	$OS=$row[4];
	echo"<tr><td>OS</td><td>$row[4]</td><td><select name=\"newos\" size=\"1\"><option value=\"TOUS\" ";
	if ($OS=="TOUS") { echo "SELECTED"; }
	echo ">TOUS</option>  <option value=\"98\" ";
	if ($OS=="98") { echo "SELECTED"; }
	echo ">Windows 98</option> <option value=\"95\" ";
	if ($OS=="95") { echo "SELECTED"; }
	echo ">Windows 95</option> <option value=\"ME\" ";
	if ($OS=="ME") { echo "SELECTED"; }
	echo ">Windows ME</option> <option value=\"Type9x\" ";
	if ($OS=="Type9x") { echo "SELECTED"; }
	echo ">Type Windows 9X</option><option value=\"NT\" ";
	if ($OS=="NT") { echo "SELECTED"; }
	echo ">Windows NT</option> <option value=\"2K\" ";
	if ($OS=="2K") { echo "SELECTED"; }
	echo ">Windows 2K</option><option value=\"XP\" ";
	if ($OS=="XP") { echo "SELECTED"; }
	echo ">Windows XP</option> <option value=\"TypeXP\" ";
	if ($OS=="TypeXP") { echo "SELECTED"; }
	echo ">Type Windows XP</option> </select></td><tr><td>".gettext("Genre")."</td>";

	echo "<td>$row[3]</td><td><select name=\"newgenre\" size=\"1\">
<OPTION value=\"REG_DWORD\" ";
	if ($row[3]=="REG_DWORD") { echo "SELECTED"; }
	echo ">REG_DWORD</option>    <OPTION value=\"REG_BINARY\" ";
	if ($row[3]=="REG_BINARY") { echo "SELECTED"; }
	echo ">REG_BINARY</option>    <OPTION value=\"REG_SZ\" ";
	if ($row[3]=="REG_SZ") { echo "SELECTED"; }
	echo ">REG_SZ</option>     <OPTION value=\"REG_EXPAND_SZ\" ";
	if ($row[3]=="REG_EXPAND_SZ") { echo "SELECTED"; }
	...
	if ($row[5]=="config") { echo "selected";} 
	echo">".gettext("config")."</option>";
	echo "<option value=\"restrict\" ";
	if ($row[5]=="restrict") { echo "selected";} 
...
case "4":
	...
	if ($newtype=="restrict") {
    	...
    	if ($newOS) {
    		$query="UPDATE corresp SET OS='$newOS' WHERE cleID='$nummodif';";
    		$resultat = mysql_query($query);
    	}
     	for ($i=0;$i<$nombre+1;$i++) {
			...
			refreshzrn($template);
			//De plus si c'est le template base qui est mis a jour on mets a jour l'ensemble des templates
			//au cas ou une cle de conf par exemple est modifiee (correction d'un bug observe)
			if ($template=="base") {
...
default:
	...
	if ($cat) {
		...
		if ($sscat) {$ajoutsscat=" AND sscat='$sscat';";
		echo "<h3> $sscat</h3>"; }
		if (!($sscat)) {$ajoutsscatvide=" AND sscat='' "; }
		$query="Select Intitule,cleID,valeur,genre,OS,antidote,type,chemin from corresp ".$ajout.$ajoutsscatvide.$ajoutsscat;
		$affichetitle="<table border=\"1\" ><tr><td><h4><img src=\"/elements/images/help.png\" alt=\"Aide\" title=\"".gettext("Aide")."\" width=\"16\" height=\"18\" border=\"0\" /></h4></td><td><DIV ALIGN=CENTER>".gettext("Intitul&#233")."</DIV></td><td>OS</td><td><DIV ALIGN=CENTER>".gettext("Valeur (defaut)")."</DIV></td><td><DIV ALIGN=CENTER>".gettext("Antidote")."</DIV><td><img src=\"/elements/images/edit.png\" alt=\"".gettext("Modifier la valeur")."\" title=\"".gettext("Modifier")."\" width=\"16\" height=\"16\" border=\"0\" /></td><td><img src=\"/elements/images/edittrash.png\" alt=\"".gettext("Supprimer")."\" title=\"".gettext("Supprimer")."\" width=\"16\" height=\"16\" border=\"0\" /></td></tr>";
		affichelisteget("affiche_cle.php?modif=3","modifkey","del",$query,$affichetitle,$testniveau);
		if ($cat=="tout") {
			$query="Select Intitule,cleID,valeur,genre,OS,antidote,type,chemin,categorie,sscat from corresp ";
			$affichetitle="<table border=\"1\" ><tr><td><h4><img src=\"/elements/images/help.png\" alt=\"".gettext("Aide")."\" title=\"$row[7]\" width=\"16\" height=\"18\" border=\"0\" /></h4></td><td><DIV ALIGN=CENTER>".gettext("Cat&#233gorie")."</DIV></td><td><DIV ALIGN=CENTER>".gettext("Sous-Cat&#233gorie")."</DIV></td><td><DIV ALIGN=CENTER>".gettext("Intitul&#233")."</DIV></td><td>OS</td><td><DIV ALIGN=CENTER>".gettext("Valeur (defaut)")."</DIV></td><td><DIV ALIGN=CENTER>".gettext("Antidote")."</DIV><td><img src=\"/elements/images/edit.png\" alt=\"".gettext("Modifier la valeur")."\" title=\"".gettext("Modifier")."\" width=\"16\" height=\"16\" border=\"0\" /></td><td><img src=\"/elements/images/edittrash.png\" alt=\"".gettext("Supprimer")."\" title=\"".gettext("Supprimer")."\" width=\"16\" height=\"16\" border=\"0\" /></td></tr>";
			affichelisteget("affiche_cle.php?modif=3","modifkey","del",$query,$affichetitle,$testniveau);
    		}
	} else {
		...
		if ($testniveau>="3") {
			...
			echo "<INPUT TYPE=\"submit\" value=\"".gettext("Ajouter une cl&#233")."\" name=\"ajoutcle\"></FORM> ";
		}
		break;
	}
mysql_close();
