...
<?php
  ...
  connexion();
if (test_bdd_registre()==false) {   exit; } else {
if (test_zorn_tools()==false) {  exit; } }
...
switch ($action) {
	...
	while ($row = mysql_fetch_array($resultat)) { echo"<a href=\"affiche_modele.php?modact=yes&mod=$row[0] \">$row[0]</a><br>"; }
	...
	//affichage d'un modele
  	...
  	if (($cat) and !($cat=="tout")) {
    		...
    		if ($_GET['sscat']) {$ajoutsscat=" AND corresp.sscat='$sscat';"; } else {$ajoutsscat=""; }
    	} else {
    	...
    	if ($cat=="tout") {
    		$ajout="";
    		if ($_GET['sscat']) {$ajoutsscat=""; }
    	}
    	if ($_GET['sscat']) { echo "<h3>".gettext("Sous-cat&#233;gorie")." $sscat</h3>"; }
    	connexion();
    	$query="SELECT `cle`,`etat` FROM `modele` WHERE `mod` = '$mod' ";
        ...
        if (!mysql_num_rows($resultat2)) {
   		...
   		$resultat6 = mysql_query($query6);
			...
			while ($row = mysql_fetch_array($resultat3)) {
					...
					if ($cat=="tout"){echo"<td><DIV ALIGN=CENTER>$row1[6]</DIV></td><td><DIV ALIGN=CENTER>$row1[7]</DIV></td>";}
...
if ($testniveau>2) {
	...
	if ($cat=="tout") {
    		$ajout="";
    		$ajoutvide="";
    		if ($sscat) {$ajoutsscatvide="";  $ajoutsscat="";}
    	}
	...
	if ($sscat) { echo "<blockquote>".gettext("Sous-Categorie")." $sscat</blockquote>"; }
	...
	if ($cat=="tout") {  echo"<td><DIV ALIGN=CENTER>".gettext("Cat&#233;gorie")."</DIV></td><td><DIV ALIGN=CENTER>".gettext("Sous-Cat&#233;gorie")."</DIV></td>";}
